...
public class Promise<T>{
	...
	protected ArrayList<callback> recievedCalls=new ArrayList<>(); //list of callbacks to be execute
	...
	public T get() {
		if(!this.isResolved())
			throw new IllegalStateException("promise didn't resolved yet");
		else
			return this.value;	
	}
	...
	public synchronized void resolve(T value) {
		if(this.isResolved()) //this has been resolved
			throw new IllegalStateException();
		this.initialized=true;
		this.value=value;
		if(!recievedCalls.isEmpty()) { //there are waiting calls
			for(callback func: recievedCalls)
				func.call();
			recievedCalls.clear();
		}
		notifyAll();
	...
	public synchronized void subscribe(callback callback) {
		if(this.isResolved())
			...
			recievedCalls.add(callback);
	}
	
	protected int callsNumber() { 
		return this.recievedCalls.size();
	}
}
